// Packs an Oklcha slot to sRGB-encoded Rgba8Unorm for display. wgpu has no
// storage binding for sRGB formats, so the gamma is applied here.
//
// mode: 0 = color, 1 = unused, 2 = scalar (L as gray),
//       3 = normal (slot holds `normal_to_color(n)`).

struct PackParams {
    size: vec2<u32>,
    mode: u32,
    // 0 = composite over the gray checker (flat previews); 1 = keep alpha
    // for the 3D preview to blend.
    alpha_object: u32,
    // mode 1 only
    const_value: vec4<f32>,
    // Volume slice index in texels; 0 for flat bakes.
    z_px: u32,
    // Not vec3<u32>: that aligns to 16 and breaks the Rust layout.
    _pad0: u32,
    _pad1: u32,
    _pad2: u32,
...
// Out-of-range values show as a magenta/black checker instead of being
// clamped. Only the preview is affected; intermediates stay unclamped.
...
// Gray checker under partial alpha. Blended in gamma sRGB, as image editors
// do, rather than in linear.
...
// Keeps f32 error at the bounds from flickering a valid image to the checker.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (params.mode == 0u) {
        ...
        // mode 3. Checked because any layer can be routed to the normal slot.
        if (oklcha_out_of_range(src_px)) {
            packed = checker_srgb(gid.xy);
        } else {
            packed = composite_alpha(oklcha_to_srgb(src_px), gid.xy);
